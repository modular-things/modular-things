...
await aMotor.setStepsPerUnit(20);

await bMotor.setCurrentScale(0.35);
await bMotor.setStepsPerUnit(20);

// we make our synchronizer object and reset the machine position to zero 
const machine = createSynchronizer([aMotor, bMotor]);

// we home it, as a treat:
await machine.absolute([110, 110])
// and we're here 
machine.setPosition([0, 0]);

console.warn("machine OK");
...
async function onCutClick() {
  ...
  console.log(state)
  for (let i = 0; i < state.selectedShapes.size; i++ ) {
    const id = [...state.selectedShapes][i];
    const shape = state.shapes[id];
    const transformed = getTransformedShape(id);
    console.log(transformed);
    for(let trace = 0; trace < transformed.length; trace ++){
      for(let pt = 0; pt < transformed[trace].length; pt ++){
        console.warn(transformed[trace][pt]);
        const [x, y] = transformed[trace][pt];
...
function readFileSVG(file) {
  ...
  reader.onloadend = event => {
    ...
    const makeNewShape = (pls) => {
      ...
      state.strokes[id] = pls.map(x => "black");
